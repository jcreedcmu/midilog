...
// MIDI file constants
// PPQ=1000, tempo=1000 us/beat → 1 tick = tempo/ppq = 1 us (exact, no rounding)
const PPQ = 1000;
const TEMPO = 1000;
...
export function eventsToMidi(events: SongEvent[]): Uint8Array {
  ...
  // MThd: "MThd", length=6, format=0, ntrks=1, ppq
  result.push(0x4d, 0x54, 0x68, 0x64);
  writeUint32BE(result, 6);
  writeUint16BE(result, 0);
  writeUint16BE(result, 1);
  writeUint16BE(result, PPQ);
...
export function midiToEvents(data: Buffer | Uint8Array): SongEvent[] {
  ...
  // Parse MThd: skip magic(4) + length(4) + format(2) + ntrks(2), read ppq
  ...
  while (offset < trackEnd) {
    ...
    // tempo/ppq = us per tick; midi_us = ticks * (tempo/ppq)
